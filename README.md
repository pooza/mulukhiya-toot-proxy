...
この為、URLの終端を正規表現 `\s` にマッチする文字（半角スペースや改行）で区切る必要がある。  
この制約は、日本語URLに対応していないMastodonには本来ないものなので、注意されたい。

### トゥート文字数について

通常のWebクライアントを含め、ほとんどのクライアントでは500文字以上のトゥートを
行えない様に文字数を監視している。  
特にアマゾンのURLを含んだトゥートは、入力時よりも長いものがトゥートできるが、
本来にはトゥート可能であるはずのテキストがクライアントによってエラーとされる
可能性がある。
