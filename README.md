...
例外発生時の通知先。Slackのwebhookと互換性のあるURLを列挙。省略可だが設定を推奨。  
...
アマゾンの商品URLを扱うという要件から、日本語を含んだURLも扱える必要があった。  
URLの終端は、正規表現 `\s` にマッチする文字（半角スペースや改行）で区切る必要がある。
この制約は、日本語URLに対応していないMastodonには本来ないもの。注意されたい。

### トゥート文字数について

通常のWebクライアントを含め、ほとんどのクライアントは500文字以上のトゥートを
行えない様に、文字数を常時監視している。  
特にアマゾンのURLを含んだトゥートは、入力時よりも長いものがトゥートできる可能性が
あるが、本来にはトゥート可能であるはずのテキストがクライアントによってエラーとされる
可能性がある。

### トゥートを改変するということ

これは「制約」とは違うかも知れないけど、トゥートに何かしらの改変をすることには違いない。  
意図に反した改変はしていないつもりだけど、特にアマゾンのアソシエイトIDにひとこと言いたい勢は
いるかもしれない。  
