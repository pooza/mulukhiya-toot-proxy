...
利用者は `main` を使用。開発は `develop` で行い、リリース時に `main` へマージする。
...
1. **本体改造を最小化する** — モロヘイヤの存在意義は、Mastodon / Misskey 本体へのパッチを増やさずにふるまいを足すこと。本体側の DB スキーマ変更は原則として行わない。パッチが増えるほど upstream 追従の工数と衝突リスクが膨らむ
2. **SNS の状態ストアには SELECT しかしない** — モロヘイヤは SNS の PostgreSQL を直読みするが、書き込まない（SNS の Redis も同様）。スキーマの所有者は SNS 側であり、SNS 自身のマイグレーションが与り知らない書き込みを外から入れると、アップグレード時の整合が壊れる

現時点の例外は Misskey の `sw_subscription` のみ（`/api/sw/register` が重複行を溜め、それを修復する API が存在しないため）。**2 つの方針が衝突した場合は、本体改造のほうを採る** — 本体を触らずに済ませる代償がモロヘイヤ側の非 SELECT では、目的と手段が逆転するため。
