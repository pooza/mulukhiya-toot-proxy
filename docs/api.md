...
- **エラー**:
  - **422**: リクエストバリデーション失敗（`episode_id` 不正など）
  ...
  - **404**（`Ginseng::NotFoundError`）: `episode_id` に対応する Annict エピソードがグローバルノード ID に解決できなかった（存在しない annictId 等）
  - **502 Bad Gateway**: 上記以外で Annict GraphQL から `errors` が返った／期待した構造の応答が得られなかった
