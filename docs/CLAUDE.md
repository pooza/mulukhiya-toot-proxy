...
## 次期マイルストーン: 5.21.0（リリース直前）

テーマ: 番組表エディタ品質確保 + 急ぎ修正（10 件）。**全 Issue 消化済、5観点並列レビュー完了。残りは赤対応コミット + version bump + ステージング検証 + リリース PR ready 化のみ**。post-release で本セクションを「リリース済み」へ移し替える。

### フェーズ3 (#4237) の扱い

5.21.0 着手前の棚卸しで、フェーズ3 のスコープ「エピソードブラウザのコピペ廃止 + 旧スプレッドシート pull 廃止」は **5.20.0 (#4236 フェーズ2) のエディタ実装で実質達成済み**と判明し B 案でクローズ。残った掃除タスクは #4308 で 5.22.0 へ。マイルストーンの目的「番組表を自ら作れる + 他モロヘイヤと共有できる」は 5.20.0 + 5.21.0 で達成済み。
...
テーマ: 5.21 レビュー黄送りの掃き出し + 番組表エディタ品質強化 + APIController リファクタ続編（17 件 / 25 重み）

### バグ修正・継続対応

- #4306 MediaCatalogUpdateWorker のクエリ劣化（cursor ページング切替・専用キュー分離、size:M）— 5.20.2 短期対処済み、根治対応
...
番組表フェーズ4（#4227, capsicum 感想投稿）はフェーズ3 達成済み（5.20.0 #4236 で実質達成 / #4237 クローズ）に伴い、5.23+ で on-hold 解除を再検討。
...
5.17.0 以前のリリースノートは [release-history.md](archive/release-history.md) を参照。
