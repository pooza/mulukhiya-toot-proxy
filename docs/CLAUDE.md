...
| 起票 | Issue | 重み | 主眼 |
| --- | --- | ---: | --- |
| 05-24 | 🎯 **#4352** | 3 | **media_catalog を shallu / gomander に横展開**。⚠ **#4639 の 24 時間観測（〜2026-09-12 05:44）が通ることが前提**。本番の設定変更なので着手前にユーザーへ確認する |
| 07-19 | #4463 | 3 | DictionaryTagHandler の投稿同期スキャンの最適化 |
| 07-21 | #4471 | 3 | `verify_host.sh` / `probe_host.sh` の判定を `stlf_probe` へ |
| 07-21 | #4476 | 3 | `stlf_probe` の絶対閾値がコンパイラでぶれる |
| 08-08 | #4543 | 3 | Sentry の未トリアージ unresolved の棚卸し |
| 08-11 | #4570 | 3 | `Program` の編集系を別クラスへ（ClassLength 上限ちょうど） |
| 08-11 | #4577 | 3 | 5.33.0 レビュー由来の観測性の穴 4 件 |
...
#### 🔴 Sentry: 今日の新規イベントは**全部**未特定の機体から出ている（→ pooza/chubo2#241）

⚠⚠ **`server_name=mulukhiya` の機体の特定を、2026-08-28 から 3 週間「次の同期へ持ち越し」にしていた。**
今回の同期で**指紋が取れた**ので chubo2 の Issue に切り出した。**もう同期のたびに調べ直さない。**
...
- ⚠⚠ **本番 4 台からの新規は 1 件も無い。**⚠ **`-2Q` を「shallu の辞書がまた全滅した」と読まないこと** ——
...
- **機体の指紋**: kernel `PMX 7.0.14-14`（2026-08-22 ビルド）/ ruby 4.0.6 / `environment=production` /
  sidekiq。🔴 **今の pve（`7.0.2-6-pve`・CT 10 台とも同一）より新しいので、この pve の上には無い。**
  FreeBSD 勢（本番 3 台 / dev24-26 / seas）・vulcan（Linode）・kues・作業用ノート（miki）のどれでもない
- ⚠ **本番の設定を持っている**（辞書ソース・カスタムフィード実体・Sentry DSN）。⚠ **障害ではないが、
  `#4543`（Sentry 棚卸し）はこの機体が混ざったままでは正しく回せない**
- 判定は Sentry の `-2Q` / `-1X` にコメントとして残した
...
5.32.x 以前のリリースノートは [release-history.md](archive/release-history.md) を参照。
