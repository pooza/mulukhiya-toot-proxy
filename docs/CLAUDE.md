...
- #4323 perf: media_attachments 関連 index 見直し（サブ #4351 zugoga 再有効化 / #4352 shallu/lbock 横展開 / #4353 本家 migration / #4375 Misskey track、1 マイルストーンに 1 件のペースで進行中）。実行 runbook は docs/media-catalog-index-plan.md（決定ゲート Gate 0〜2 + rollback）。Mastodon は statuses partial index、Misskey は別ルート（drive_file、daisskey で実証済み）
...
- [test-harness.md](test-harness.md) — #4379 chubo2 fedi-test-harness を使った実インスタンステストの手順
