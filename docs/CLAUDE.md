...
### gomander は「Linode が遅い」のではなく「ハズレ個体」（2026-07-20 確定）

gomander に rbenv + Ruby 4.0.5（YJIT built=true）を導入し（chubo2#72）、本番と同じ条件で測り直した結果、**zugoga と gomander は同一 CPU（AMD EPYC 7713）・同一 2 コア・同一 Ruby / YJIT なのに 1.52 倍の差**があった。Linode や EPYC 一般のペナルティではなく、gomander という個体が遅い。

さらに分散サンプラ（3.4.9・10 分毎）の初期データでは、lbock 約 6% / zugoga 約 7% の揺れに対し **gomander は約 1.6% と極めて安定**していた。隣人輻輳なら分散が大きく出るはずで、そうなっていない。**「gomander は安定して遅いのか揺れているのか」は『安定して遅い』で決着。**

示唆は **プランアップより先に gomander の作り直し（別の物理ホストへの着地）を試すべき**ということ。同一プランの zugoga が lbock と互角である以上、現行スペックでも性能要件は満たせる公算が高い。8GB 化は「ニチアサ中に Sidekiq のキューが詰まっているか」という別の根拠で判断する。受け皿は chubo2#68。
...
判定したいこと: ①gomander は安定して遅いのか揺れているのか ②zugoga / lbock が実況時間帯に劣化するか（Shared 契約が実況ウィンドウで牙を剥くか）。**プラン見直し・カットオーバー（chubo2#68）の判断材料**であり、07-26 の観測後に読む。
