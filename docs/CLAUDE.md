...
開発者本人（pooza）は概ね毎晩プリキュアを視聴し、視聴直後に感想を書く活動を数年継続している。番組表・実況機能、capsicum のエピソードブラウザ、Annict 連携、エピソード感想投稿などの機能群はすべてこのワークフローを支えるために作られており、プロダクト設計の中心的な駆動力。
...
テーマ: **番組表リニューアル フェーズ4 達成**（Annict 感想投稿 API） + 5.21 レビュー黄送りの掃き出し + 番組表エディタ品質強化 + 5.21.x ホットフィックス Codex 指摘対応（17 件 / 29 重み — 予算超過 +4、Annict 主軸の進捗を見て #4271/#4278/#4279 のいずれかを 5.23.0 へ送る含み）— **2026-05-07 全件 develop 反映済み**

### リリース前 5観点並列レビュー対応（2026-05-07）

- **赤 3 件 + 黄 1 件昇格**: SidekiqDaemon.health の capsule 反映、refines.rb の Sentry capture rescue、docs/api.md の 401→403 訂正、AnnictService の非 Hash response 防御
...
- **黄 4 件・緑 4 件は Issue 起票で送り**: #4328 (Y3) #4329 (Y5) #4330 (Y6) #4331 (Y7) #4332 (G1) #4333 (G2) #4334 (G3) #4335 (G4)
...
- #4322 perf: media_catalog 専用 Sidekiq キュー分離（default キュー詰まり防止、#4306 中期2、size:M）— 5.21.2 で cursor 化済み、本リリースで専用キュー分離 — **2026-05-07 develop 反映済み**（Sidekiq Capsule で media_catalog キュー分離、concurrency 1）
- #4325 fix: MediaCatalogUpdateWorker の cursor が Misskey の非ユニーク順序で添付欠落（5.21.2 PR #4324 への Codex P1、size:M）— **2026-05-07 develop 反映済み**（短期対処: Misskey では cursor 無効化し OFFSET ページング維持。SQL の複合キー cursor 化は #4323 と合わせて将来検討）
...
テーマ: 調査系 + 番組表エディタ拡張 + 表記改善・小掃除 + 5.22 レビュー送り（15 件 / 27 重み — 予算超過 +2 だが許容範囲）
...
- #4272 auto_update 有効時は番組表エディタを参照専用にする（size:S）
...
- #4328 AnnictService の GraphQL エラーをカテゴリ別 status code で返す（Y3、size:S）
- #4331 POST /annict/record に冪等性を持たせて重複 record 投稿を防ぐ（Y7、size:M）
- #4332 RemoteHost.public? の bare rescue を具体例外に絞る（G1、size:S）
...
番組表フェーズ4（#4227, capsicum 感想投稿）はフェーズ3 達成（5.20.0 #4236 で実質達成 / #4237 クローズ）に伴い on-hold 解除済み、5.22.0 主軸に組み込み。capsicum 側は pooza/capsicum#298（v1.26）でペア対応中。
...
- #4329 perf: HTTP fetch のサイズ検証を Content-Length 事前判定に切替（5.22 Y5 送り、size:M）
- #4330 feat: Addrinfo.getaddrinfo にタイムアウトを設定し Puma スレッド枯渇を防ぐ（5.22 Y6 送り、size:M）
- #4333 feat: SwSubscriptionContract の allowed_hosts 空配列を deny-all に反転（5.22 G2 送り、運用変更要、size:S）
- #4334 perf: RateLimitStorage を EVALSHA + NOSCRIPT フォールバックに移行（5.22 G3 送り、size:S）
- #4335 refactor: MediaCatalogUpdateWorker の cursor_pagination? を Attachment 側に移譲（5.22 G4 送り、#4323 と合わせて、size:S）
...
5.18.0 以前のリリースノートは [release-history.md](archive/release-history.md) を参照。
