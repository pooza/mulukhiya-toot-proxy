...
GitHub マイルストーン作成・割り当て済み（2026-07-20）。重み合計 **17**（`size:M` × 5、`size:S` × 2）。
...
### 同梱: Ruby ランタイムの能力欠落を検知する（#4466 / #4467 / chubo2#69）

**YJIT は Rust がないとビルド時に黙って外れる。** エラーにならないため、能力を欠いた Ruby が完成しそのまま本番投入されうる。2026-07-20 の調査で実際に起きていることを確認した。

| ホスト | Rust | Ruby 4.0.5 の YJIT | 実行時 |
| --- | --- | --- | --- |
| lbock / zugoga / shallu | あり | ✅ ビルド済み | **無効（未有効化）** |
| gomander | **なし** | 4.0.5 未導入。積むと YJIT なしになる | — |

本番 4 台は YJIT 込みでビルドしているのに実行時に有効化していない（`RUBYOPT` は bundler の setup のみ）。YJIT は 4.0.5 でもデフォルト無効。

**YJIT の効果は `raw_cpu` で 24〜25% 短縮**（lbock 1558→1184ms、zugoga 1603→1202ms）。ただし `regexp_compile` / `sweep` / `marshal_load` はほぼ不変で、正規表現エンジンも Marshal も C 実装のため YJIT の対象外。**数秒の解決策ではなく、無料で取れる 24%** という位置づけ。
...
- **#4467 perf: 本番で YJIT を有効化する（size:M）** — メモリ増を実測してから。4GB 機でキュアスタ！は過去に RAM 逼迫の実績あり
...
| ホスト | 3.4.9 | **4.0.5 YJIT なし＝本番の現状** | 4.0.5 YJIT あり |
| --- | --- | --- | --- |
| lbock（さくら） | 1408 ms | **1558 ms** | 1184 ms |
| zugoga（Linode） | 1624 ms | **1603 ms** | 1202 ms |
| gomander | 2515 ms | 4.0.5 未導入 | Rust なしで不可 |

**本番 Ruby では lbock と zugoga の差は 1.03 倍＝実質同等。** 3.4.9 で見えた 1.15 倍の Linode ペナルティは本番条件ではほぼ消える。「さくらは価格性能比で優れる」は、少なくとも per-core CPU については本番条件で裏付けられない。**gomander だけが 1.55 倍遅い異常個体**という構図が残る。
...
上記実測のとおり gomander は per-core 1.7 倍遅くコアも半分のため、**プラン見直しの検討が必要**（2026-07-20 時点でユーザーに考えがあり、相談待ち）。
