...
**進捗（2026-05-23 時点）**: 16/17 件 closed、残 1 件 open（CI 緑）。closed: #4280 #4314 #4319 #4313 #4316 #4333 #4334 #4335（〜2026-05-18 着地、手動検証価値が薄いためリリース前倒し）、#4318 #4328 #4329 #4330 #4331 #4338（2026-05-21 ステージング検証後クローズ、#4335 #4331 は CI で test-unit 環境差由来の追従修正あり）、#4332（allow-all デフォルトが妥当 — endpoint がベンダー管理で allowlist 列挙不能・内部宛 SSRF は #4271 で対応済み、**wontfix クローズ**、NOTE コメントに理由明記）、#4336（2026-05-23 ステージング動作確認後クローズ）。**スコープ変更（2026-05-23）**: #4323 は zugoga 本番ベースライン EXPLAIN で病理を確証（§2-1 = 175,248 ms、candidate A の partial index 適用方針も確定）したが、複数本番への段階的展開で 1〜2 週間のバースト作業スケール。media_catalog 機能を **実験的扱いでデフォルト無効化**する判断に切り替え、本来の最適化 #4323 は on-hold へ移動（ベースラインと candidate A は再開時の起点として残す）。代替着地 Issue として #4343 を新規追加。open 維持: #4343（feat: media_catalog をデフォルト無効化 + クライアント向け disabled シグナル、size:M）。リリース PR #4341 は DRAFT 状態、`config/application.yaml` のバージョンは 5.22.1 のまま。
...
### 番組表エディタ補助・media_catalog 扱い変更

- #4336 番組表エディタの各エントリに作品名・話数+サブタイトルのコピーボタンを追加（毎朝の挨拶投稿運用の手数削減、#4286 の代替最小実装、ハッシュタグ列は本体ハンドラで自動付与済みのためスコープ外、size:S）→ **着地・クローズ済み**
- #4343 feat: media_catalog をデフォルト無効化（実験的機能扱い）+ クライアント向け disabled シグナル（features API に `media_catalog` 露出 / disabled 時 503 / WebUI placeholder、size:M）。capsicum 側は pooza/capsicum#606 で gate 追加、同時または直後にリリース
- #4338 feat: features API に `annict_linked`（ユーザーの Annict 連携済みフラグ）を追加（capsicum 連携、size:S）→ **着地・クローズ済み**

本来の最適化 #4323（media_attachments 関連 index 見直し）は on-hold へ移動。zugoga 本番ベースライン EXPLAIN と candidate A の方針確定済み、media_catalog 機能を再開する判断時に着手する。

## 次々期マイルストーン: 5.24.0

テーマ: 番組表エディタ拡張 + 調査・運用検証 + リファクタ（7 件 / 19 重み）。5.23 のレビュー送り消化分を見越したバッファあり。5.23 リリース前後で 5.22 緑送り後発・5.23 レビュー送りを取り込む想定。
...
- #4323 perf: media_attachments 関連 index 見直し（#4343 で機能をデフォルト無効化したため急ぎでなくなった。zugoga 本番ベースライン EXPLAIN + candidate A の方針は確定済み、機能再開判断時に着手）
