...
**2026-09-08 に 5.36.0 を出荷した直後の状態。**マイルストーンは未作成。

⚠ **消化を目的にした回にする方針**（2026-09-08 のユーザーとの整理）。テーマで選ばず、**年齢と重みで選ぶ**。
...
- **今回のレビューが出した実バグ 3 件** — #4693（`CryptError` の全滅が Sentry 無音）/
  #4694（タイムアウトで添付ドロップが応答にも syslog にも残らない）/ #4696（ffmpeg の内側 Timeout が構造的に発火しない）
- **90 日超の古株**から重み 20〜25 まで
