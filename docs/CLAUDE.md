...
テーマ: 5.22 リリース前レビュー送り消化 + 観測性・docs 改善 + 番組表エディタ補助機能（16 件 / 24 重み — 予算 25 内）。番組表リニューアル全フェーズ完了後の最初のリリースで、主軸は据えず「滞留した小粒の整理回」と位置付ける。
...
- #4336 番組表エディタの各エントリに作品名・サブタイトル・タグのコピーボタンを追加（毎朝の挨拶投稿運用の手数削減、#4286 の代替最小実装、size:S）
