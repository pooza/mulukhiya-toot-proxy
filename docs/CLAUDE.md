...
- #4143 ヘルスチェックにストリーミングプロセスの死活監視を追加 — #4142 で一時除外したストリーミング監視を、SNSのヘルスエンドポイント疎通確認で再導入
- #4145 Lemmy対応を廃止し、PiefedClipperを自立化 — 利用者がPieFedに移行済みのため不要。`LemmyClipper` 削除、設定削除
