...
**2026-08-10 時点の実測**: `rake test` **954 tests / 1292 assertions / 0 failures / 0 errors / 313 omissions**。
本日 7 PR ぶんでテストは 932 → 954 に増えたが **omission は 313 のまま**なので、CI の
`omission_baseline`（mastodon 313 / misskey 302）は据え置きでよい。

**リリースまでに残る手順**: #4534 の要否判断 → **harness 実走ゲート（両系 0 failures / 0 errors・省略不可）**
