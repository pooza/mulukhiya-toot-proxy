...
2. **移行の判断材料の獲得** — キュアスタ！(lbock, さくら VPS) を gomander (Linode/FreeBSD 15) へ移す後続タスクに関わる

### コスト制約（最重要・順序を規定する）

**gomander と lbock の二重ランニングコストが数ヶ月以上発生している。** 出せない額ではないが長期化しており、カットオーバーの前倒しが最優先事項。したがって **5.30.0 の完了をカットオーバーの前提にしない**。

**切り分けの懸念を解消するのは最適化（#4463/#4465）ではなく計装（#4464）である。** ハンドラ別の所要時間が両機で取れていれば、移行後に遅くなったとき「gomander の素の性能」か「元からの dictionary_tag」かを正確に帰属できる。最適化の完了を待つ必要はない。

**gomander は現在アイドルで課金されている＝そのまま性能ラボとして使える。** lbock 本番（実負荷）と gomander（合成負荷）の両方に計装を入れて比較する。

**進行**:

1. **#4464 計装を最優先で仕上げる**（唯一の真のブロッカー）
2. lbock / gomander 両機で pre_toot 内訳を取得・比較
3. 分岐:
   - **gomander ≧ lbock** → **即カットオーバー**して二重課金を止め、#4463 は gomander 上で後追い
   - **gomander が明確に劣る** → そこで初めて #4463 A/B/C がゲート（それでも size:M）
4. **#4465 はカットオーバーのクリティカルパスに置かない**

gomander は FreeBSD 15 + ZFS + PostgreSQL 18 + pgbouncer の新構成で、lbock（さくら VPS、旧世代 PostgreSQL）より速い可能性のほうが高い。その場合に最適化を待つ判断は、二重課金を一リリースサイクル分ドブに捨てることになる。

なお MRI/GVL のため pre_toot 直列処理は 1 コア律速であり、**ハード増強では単投稿は速くならない**（#4463 の UX 価値は移行先に関わらず不変）。

**Issue:**

- **#4464 perf: `pre_toot` ハンドラの所要時間を計装する（size:M）** — 🔴 **カットオーバーの唯一のブロッカー。最優先。** lbock 本番と gomander の両方で内訳を取る
- **#4463 perf: DictionaryTagHandler の投稿同期スキャン最適化（size:M）** — A. `addition_tags` メモ化（3回→1回）/ B. 解析済み辞書のプロセス内キャッシュ / C. `short?` の Regexp 事前コンパイル。**カットオーバー非ブロッカー**（gomander が lbock に明確に劣った場合のみゲート化）
- **#4465 perf: `TaggingDictionary#matches` のアルゴリズム刷新（size:L）** — 線形スイープ O(語数 × テキスト長) を索引化（trie / Aho-Corasick）。**カットオーバー非ブロッカー（クリティカルパスに置かない）**。#4464 の計装結果を見て着手可否を決めるゲート付きで、寄与が小さい／A/B/C で足りるなら 5.31.0 へ緑送り
