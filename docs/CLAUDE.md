...
- 🔴 **`ginseng-fediverse` v1.8.31 → v2.0.1 は「取り込む」に転じた（2026-09-20 の同期・ユーザー確認待ち）。**
  **v2.0.1 が 2026-09-17 に出た**が、⚠⚠ **待っていた pooza/ginseng-fediverse#276 は塞がっていない（今も open）。**
  にもかかわらず**保留を続ける理由が無くなった**のは、v2.0.1 に**保留の根拠より重いものが 2 つ入った**から:
  ...
  - ⚠ **取り込むときは `test/unit/lib/string.rb:9` の `assert_equal('IDOLM@ STER', 'IDOLM@STER'.escape_toot)` を
    必ず一緒に直す。壊れた側を期待値に固定している**ので、直った gem を入れるとここだけ落ちる
  - ⚠ **`bundle update ginseng-fediverse` と gem 単位で上げる**（8 本まとめて動かさない）
...
#### 🔴 判断が要る 1 件: `ginseng-fediverse` v2.0.1

**保留を解いて 5.38.0 で取り込むことを提案する。**根拠は上の「取り込み済み」節に書いた
（#278 の投稿リダイレクト＋#273 の `@` 過剰エスケープ、#276 は本番の口に当たらない）。
⚠ **これはリリースのスコープを動かす話なのでユーザー判断に残す。**
