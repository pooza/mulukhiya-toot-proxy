...
テーマ: 番組表エディタ拡張 + 調査・運用検証 + リファクタ + 5.23 レビュー送り消化（8 件 / 21 重み）。
...
テーマ: APIController 段階的リファクタの締め + 5.23 レビュー送りの構造改善（暫定 3 件 / 13 重み）。
...
5.23 レビュー送りの残り（#4346 alert しきい値・#4349 Renderer 切り出し）は未設定マイルストーンに残置、5.25 以降に必要時取り込む。
...
- #4346 feat: AnnictRecordLockStorage 冪等性ロックの異常頻度を Sentry に alert（5.23 レビュー送り、size:M）
- #4349 refactor: /media disabled 応答を MediaCatalogDisabledRenderer に切り出し（5.23 レビュー送り、size:S）
...
5.20.x 以前のリリースノートは [release-history.md](archive/release-history.md) を参照。
