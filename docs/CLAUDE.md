...
マイルストーンの Issue は全消化（open 0 件）。ステージング検証も 2026-08-06 に一度通っている。
ただし**この検証で出荷が確定したわけではない**——次のレビューで修正が入るため、
検証はその後にもう一度必要になる。

1. **リリース前 5 観点レビュー** — 修正が出る前提で見る
2. **ステージング再デプロイ・再検証（省略不可）** — dev24-27 全 4 台。**レビューで入った修正を載せた状態で通す**。
   省略した回に本番障害を出している（MEMORY `project_5280-staging-skip-postmortem`）。
   08-06 の検証はレビュー前の develop に対するもので、これの代わりにはならない
3. バージョンバンプ → リリース PR → 本番 4 台デプロイ
