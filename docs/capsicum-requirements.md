...
- エンドポイント案: `GET /mulukhiya/api/roles/admin`（または既存エンドポイントへの追加）
- 認証: 不要（ロール ID 自体は機密情報ではない）
- レスポンス例:
...
- Mastodon / Misskey 両対応（Misskey は `isAdministrator` フィールドがあるため不要だが、統一 API として提供されるなら利用する）
