...
BASES = ['プリキュア', '変身', 'バンク', '実況', '日曜', '朝', '光', '星', '空', '花', '風'].freeze
...
patterns = words.map {|w| Regexp.new(Regexp.escape(w))}
...
blob = Marshal.dump(words.to_h {|w| [w, {words: [w], pattern: w}]})
report['marshal_load'] = Benchmark.realtime do
  # 計測対象そのものが Marshal.load（自前で dump した blob のみ）
  50.times {Marshal.load(blob)} # rubocop:disable Security/MarshalLoad
end / 50
...
report.each {|k, v| puts '%-22s %8.1f ms' % [k, v * 1000]}
